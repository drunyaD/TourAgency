...
namespace TourAgency.WEB.Controllers
{
    ...
    public class UsersController : ApiController
    {
        ...
        public HttpResponseMessage Login([FromBody]LoginModel model)
        {
            if (model == null)
            {
                return Request.CreateResponse(HttpStatusCode.BadRequest, "LoginModel must be attached");
            }
        ...
        public HttpResponseMessage Register([FromBody]RegisterModel registerModel)
        {
            ...
            return Request.CreateResponse(HttpStatusCode.Created, new UserModel {
                Id = userId, Email = user.Email, UserName = user.UserName, CityId = user.CityId
            });
        }
        ...
        public HttpResponseMessage ChangeRole([FromUri]string userId, [FromBody]RoleModel roleModel)
        {
            try
            {
                Service.ChangeRole(userId, new RoleDto { Name = roleModel.Name});
            }
        ...
        public HttpResponseMessage GetUser(string userId)
        {
            ...
            if (user.UserName == principal.Identity.Name || principal.IsInRole("administrator")
